...
if ($method === 'GET') {
    ...
    mysqli_stmt_fetch($dStmt);
    mysqli_stmt_close($dStmt);
    if (!$message) { http_response_code(400); echo json_encode(['error' => 'Message obligatoire']); exit; }
    if (!$project_id) { http_response_code(400); echo json_encode(['error' => 'project_id manquant']); exit; }
    if (!$developpeur_id) { http_response_code(400); echo json_encode(['error' => 'id_developpeur manquant']); exit; }

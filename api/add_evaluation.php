...
$stmt = mysqli_prepare($conn, "
    INSERT INTO evaluations 
        (id_projet, id_evalue, type_evaluateur, note, commentaire, date_evaluation)
    VALUES (?, ?, ?, ?, ?, NOW())
");
mysqli_stmt_bind_param($stmt, "iisis", $id_projet, $id_evalue, $type_evaluateur, $note, $commentaire);

if (mysqli_stmt_execute($stmt)) {

    // Si développeur évalue entrepreneur
    ...
    // Si entrepreneur évalue développeur
    if ($type_evaluateur === 'entrepreneur' && $user_id) {
        $upd = mysqli_prepare($conn, "
            ...
            WHERE project_id = ? AND developpeur_id = ?
        ");
        mysqli_stmt_bind_param($upd, "ii", $id_projet, $id_evalue);
        mysqli_stmt_execute($upd);
        mysqli_stmt_close($upd);
    }

...
if ($method === 'GET') {
    ...
    mysqli_stmt_bind_param($stmt, "isssssisss",
        $entrepreneurId, $titre, $description, $budget,
        $duree, $statut, $id_categorie, $competences, $publier
    );
    ...
    echo json_encode(["success" => true]);
        ...
        mysqli_stmt_bind_param($stmt, "ssssssisi",
            $nom, $publier, $budget, $duree, $competences,
            $statut, $categorie, $description, $projectId
        );
        if (mysqli_stmt_execute($stmt)) {
            echo json_encode(["success" => true]);
        } else {

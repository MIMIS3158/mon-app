...
if ($method === 'GET') {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $workshops[] = [
            ...
            'description'      => $row['description'],
            //'thumbnail'        => $row['thumbnail'] ?: 'assets/workshop-default.jpg',
            //'thumbnail' => $row['thumbnail'] ? 'http://localhost:8000/' . $row['thumbnail'] : null,
...
if ($method === 'GET') {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $workshops[] = [
            ...
            'title'            => $row['title'],
            'description'      => $row['description'],
            'thumbnail'        => $thumb ? $thumb : null, // ✅ URL directe
            'video_url'        => $row['video_url'],
            'duration'         => $row['duration'],
            'date'             => $row['date'],
            'price'            => (float)$row['price'],
    ...
    $description = $data['description'] ?? null;
    $video_url   = $data['video_url'] ?? null;
    $thumbnail   = $data['thumbnail'] ?? null; // ✅ AJOUTÉ
    $duration    = $data['duration'] ?? null;
    $date        = $data['date'] ?? null;
    if ($date && strpos($date, 'T') !== false) {

        $date = str_replace('T', ' ', $date) . ':00'; // ✅ format MySQL
    }
    $price    = $data['price'] ?? 0;
    $max_part = $data['max_participants'] ?? 50;
    $is_free  = $data['is_free'] ?? false;
    ...
    $stmt = mysqli_prepare($conn, "
        INSERT INTO workshops 
        (entrepreneur_id, title, description, video_url, thumbnail, duration, date, price, order_num, max_participants, is_free)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");

    $is_free_int = $is_free ? 1 : 0;
    mysqli_stmt_bind_param($stmt, "issssssdiis",
        $entrepreneur_id, $title, $description,
        $video_url, $thumbnail, $duration, $date,
        $price, $order_num, $max_part, $is_free_int
    );
    ...
    $stmt = mysqli_prepare($conn, "
        UPDATE workshops SET
        title = ?, description = ?, video_url = ?,
        thumbnail = ?, duration = ?, date = ?, price = ?,
        max_participants = ?, is_free = ?
        WHERE id = ?
    ");
    mysqli_stmt_bind_param($stmt, "sssssssdii",
        $title, $description, $video_url,
        $thumbnail, $duration, $date, $price,
        $max_part, $is_free_int, $id
    );

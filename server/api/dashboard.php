...
// Projets publiés
$stmt2 = mysqli_prepare($conn, "SELECT COUNT(*) FROM projects WHERE entrepreneur_id = ?");
mysqli_stmt_bind_param($stmt2, "i", $entrepreneur_id);
mysqli_stmt_execute($stmt2);
mysqli_stmt_bind_result($stmt2, $total_projets);
...
$stmt3 = mysqli_prepare($conn, "
    SELECT COUNT(*) FROM candidatures c
    JOIN projects p ON c.project_id = p.id
    WHERE p.entrepreneur_id = ? AND c.statut = 'Terminée'
");
mysqli_stmt_bind_param($stmt3, "i", $entrepreneur_id);
mysqli_stmt_execute($stmt3);
mysqli_stmt_bind_result($stmt3, $missions_terminees);
mysqli_stmt_fetch($stmt3);
mysqli_stmt_close($stmt3);

// Compétences requises
$stmt4 = mysqli_prepare($conn, "
    SELECT Competences FROM projects
    WHERE entrepreneur_id = ? AND Competences IS NOT NULL
");
mysqli_stmt_bind_param($stmt4, "i", $entrepreneur_id);
...
$stmtActifs = mysqli_prepare($conn, "
    SELECT COUNT(*) FROM projects 
    WHERE entrepreneur_id = ? AND Statut != 'terminé'
");
mysqli_stmt_bind_param($stmtActifs, "i", $entrepreneur_id);
mysqli_stmt_execute($stmtActifs);
mysqli_stmt_bind_result($stmtActifs, $projets_actifs);
mysqli_stmt_fetch($stmtActifs);
mysqli_stmt_close($stmtActifs);

// Missions actives
$stmtMissions = mysqli_prepare($conn, "
    SELECT COUNT(*) FROM missions 
    WHERE entrepreneur_id = ? AND statut != 'terminée'
");
mysqli_stmt_bind_param($stmtMissions, "i", $entrepreneur_id);
mysqli_stmt_execute($stmtMissions);
mysqli_stmt_bind_result($stmtMissions, $missions_actives);
mysqli_stmt_fetch($stmtMissions);
mysqli_stmt_close($stmtMissions);

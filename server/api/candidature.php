...
if ($method === 'GET') {
    ...
    if (!$project_id && !$mission_id) {
        http_response_code(400);
        echo json_encode(['error' => 'project_id ou mission_id manquant']);
        exit;
    }
    if (!$developpeur_id) {
        http_response_code(400);
    ...
    if ($project_id) {
        ...
        $check = mysqli_prepare($conn, "SELECT id FROM candidatures WHERE developpeur_id = ? AND mission_id = ?");
        mysqli_stmt_bind_param($check, "ii", $developpeur_id, $mission_id);
    }
    mysqli_stmt_execute($check);
    mysqli_stmt_store_result($check);
    if (mysqli_stmt_num_rows($check) > 0) {
        http_response_code(409);
        echo json_encode(['error' => $project_id ? 'Vous avez déjà postulé à ce projet' : 'Vous avez déjà postulé à cette mission']);
        exit;
    }
    mysqli_stmt_close($check);
    ...
    $stmt = mysqli_prepare($conn, "
        INSERT INTO candidatures 
            (project_id, mission_id, developpeur_id, message, budget_propose, duree_estimee, statut, date_postulation)
        VALUES (?, ?, ?, ?, ?, ?, 'En attente', NOW())
    ");
    $stmt->bind_param("iiisss", $project_id_val, $mission_id_val, $developpeur_id, $message, $budget, $duree_estimee);
